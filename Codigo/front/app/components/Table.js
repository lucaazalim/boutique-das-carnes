    ...
    return (
        <>
            <table className='w-full'>
                ...
                <tbody>
                    {(data.fornecedores && data.fornecedores.length > 0) && data.fornecedores.map(fornecedor => (
                        <tr key={fornecedor.id} className='grid grid-cols-8 border-0 border-b '>
                            ...
                            <td className='col-span-1 flex justify-center'>
                                <Link href={`fornecedores/${fornecedor.id}`} legacyBehavior>
                                    <a className='text-center p-2 col-span-1 bg-yellow-400 w-[80%] rounded-lg'>Editar</a>
                                </Link>
                            </td>
                        </tr>
                    ))}
                    {(data.compras && data.compras.length > 0) && data.compras.map(compra => (
                        <tr key={compra.id} className='grid grid-cols-8 border-0 border-b '>
                            ...
                            <td className='p-2 col-span-2 border-l flex justify-center'>{compra.id_fornecedor}</td>
                            <td className='p-2 col-span-2 border-l flex justify-center'>{compra.unidades_macho + compra.unidades_femea}</td>
                            <td className='p-2 col-span-2 border-l border-r flex justify-center'>{(compra.peso_total_abate * compra.preco_arroba) - compra.desconto}</td>
